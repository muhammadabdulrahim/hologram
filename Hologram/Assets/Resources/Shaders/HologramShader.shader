	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				o.screenuv = ((o.vertex.xy / o.vertex.w) + 1) / 2;//Deferred?
			...
			fixed4 texColor(v2f i, float rim)
			{
				...
				mainTex.r *= triWave(_Time.x * 5, i.objectPos.y, -0.7) *6;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 overlay = tex2D(_MainTex, i.uv) * _Color;
				fixed4 niceColor = fixed4(lerp(_Color.rgb, fixed3(1,1,1), pow(intersect, 4)), 1);
				fixed4 col = _Color * _Color.a + niceColor * intersect + overlay;
				return col;
